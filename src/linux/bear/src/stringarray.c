...
char const * bear_strings_fold(char const * const * in, char const separator)
{
    ...
    for (; (it) && (*it); ++it)
    {
        ...
        acc = (char *)realloc(acc, acc_size + sep + it_size + 2);
        if (0 == acc)
        {
            perror("xcalbuild: realloc");
            exit(EXIT_FAILURE);
...
char const ** bear_strings_unfold(char const * in, char const separator)
{
    char * str = strdup(in);
    if (0 == str)
    {
    ...
    for (; (it) && (*it); ++it)
    {
        ...
        {
            *it = '\0';
            result = bear_strings_append(result, start);
            start = it + 1;
        }
    }
    // The last one.
